...
/**
  ...
  */
class PSOCluster(centroid: Array[Double], dataSet: DataSet) extends CentroidCluster(centroid, dataSet){


  def moveCenter: PSOCluster = {
    require( members.nonEmpty, s"Cannot move the center of an empty cluster")

    // Creates the new cluster
    val newCluster = new PSOCluster(stats.means,dataSet)
